...
auto get_uci_section_for_name(const std::string & name) -> uci::section
{
    ...
    auto errmsg = std::string{"lookup error: there is no file named '"};
    errmsg += std::string{CONF_DIR} + name + ".conf' and the UCI config ";
    if (uci_enabled) {
        errmsg += "has no nginx server with UCI name or 'server_name': " + name;
    } else {
        errmsg += "is not enabled by:\n\tuci set nginx.global.uci_enable=true";
    }
    throw std::runtime_error(errmsg);
}
...
inline auto del_ssl_from_config(const std::string & name)
{
    ...
    errmsg += ": uci set nginx."+secname+"."+MANAGE_SSL.data()+"='self-signed'";
    throw std::runtime_error(errmsg);
}
