                              ...
                              const std::string_view key="")
{
    ...
    if (!crt.empty() && !key.empty()) {
        sec.set("ssl_certificate", crt);
        std::cerr<<"\tssl_certificate='"<<crt<<"'\n";
        sec.set("ssl_certificate_key", key);
                       ...
                       const std::string_view keypath)
{
    if (crtpath[0]!='/') {
        ...
        errmsg += "path cannot be relative '" + std::string{crtpath} + "'";
        throw std::runtime_error(errmsg);
    }

    if (keypath[0]!='/') {
        auto errmsg = std::string{"add_ssl_if_needed error: path to ssl_key "};
        errmsg += "cannot be relative '" + std::string{keypath} + "'";
        throw std::runtime_error(errmsg);
    }
    ...
    if (access(legacypath.c_str(), R_OK)!=0) {
        add_ssl_to_config(name, manage, crtpath, keypath); // let it throw.
        return;
    } // else:

    auto crt = std::string{CONF_DIR} + name + ".crt"
    if (symlink(crtpath.data(), crt.c_str()) != 0) {
        auto errmsg = std::string{"add_ssl_if_needed error: cannot link "};
        errmsg += "ssl_certificate " + crt + " -> " + crtpath + " (";
        errmsg += std::to_string(errno) + "): " + std::strerror(errno);
        throw std::runtime_error(errmsg);
    }

    auto key = std::string{CONF_DIR} + name + ".key"
    if (symlink(keypath.data(), key.c_str()) != 0) {
        auto errmsg = std::string{"add_ssl_if_needed error: cannot link "};
        errmsg += "ssl_certificate_key " + key + " -> " + keypath + " (";
        errmsg += std::to_string(errno) + "): " + std::strerror(errno);
        throw std::runtime_error(errmsg);
    }
...
//     if (lstat(crtpath.c_str(), &sb) != 0 ||
//         sb.st_mode & S_IFMT != S_IFLNK ||
//         remove(crtpath.c_str()) != 0)
//     {
//         auto errmsg = "del_ssl warning: cannot remove "+crtpath;
//         perror(errmsg.c_str());
//     }
...
//
//     if (lstat(keypath.c_str(), &sb) != 0 ||
//         sb.st_mode & S_IFMT != S_IFLNK ||
//         remove(keypath.c_str()) != 0)
//     {
//         auto errmsg = "del_ssl warning: cannot remove "+keypath;
//         perror(errmsg.c_str());
//     }
